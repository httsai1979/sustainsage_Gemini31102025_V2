...
import { validateServiceContent } from './schema/serviceSchema';
...
export function getServiceData(slug, locale = 'en-GB') {
  ...
  // Validate the content using Zod
  const validatedData = validateServiceContent(data, usedLocale);
...
export function getServiceCards(locale = 'en-GB') {
  ...
  const fallbackNotice =
    slugResults.map((result) => result.service?.fallbackNotice).find((value) => typeof value === 'string' && value.length > 0) ??
    null;
  ...
  return { cards, fallbackUsed, fallbackNotice };
}
