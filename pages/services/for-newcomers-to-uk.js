...
export default function ForNewcomersToUkPage() {
  ...
  const content = t('newcomers', { returnObjects: true }) ?? {};
  ...
  return (
    <>
      <Head>
        <title>{seo?.title ?? 'Coaching for newcomers to the UK'}</title>
        <meta
          name="description"
          content={seo?.description ?? 'Coaching support for people settling into UK work and life.'}
        />
      </Head>

      <Hero
        title={hero?.title ?? 'Coaching for newcomers to the UK'}
        subtitle={hero?.subtitle ?? ''}
        image="/images/services/new-to-uk.svg"
        imageAlt={hero?.imageAlt ?? 'Illustration representing newcomers to the UK'}
      >
        <Link
