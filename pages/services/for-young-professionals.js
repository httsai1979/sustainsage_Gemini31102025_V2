...
export default function ForYoungProfessionalsPage() {
  ...
  const content = t('youngProfessionals', { returnObjects: true }) ?? {};
  ...
  return (
    <>
      <Head>
        <title>{seo?.title ?? 'Coaching for young professionals'}</title>
        <meta
          name="description"
          content={seo?.description ?? 'Coaching designed for early-career professionals.'}
        />
      </Head>

      <Hero
        title={hero?.title ?? 'Coaching for young professionals'}
        subtitle={hero?.subtitle ?? ''}
        image="/images/services/experiments.svg"
        imageAlt={hero?.imageAlt ?? 'Illustration of a young professional exploring options'}
      >
        <Link
